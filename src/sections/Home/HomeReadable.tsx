  ...
  return (
    <section className="Home_Readable border mt-3 mb-5 px-3 py-4">
      {HomeRedableData.map((read, index) => (
        <div key={index}>
          {read.title.length > 0 && (
            <h2 className="fw-bolder mb-3 CDEFAULT">{read.title}</h2>
          )}
          {read.titleBlue.length > 0 && (
            <h3 className="fw-bolder text-primary mb-3 CURSOR">
              {read.titleBlue}
            </h3>
          )}
          ...
          <p className="Home_Readable_lorem text-muted lead CDEFAULT">
